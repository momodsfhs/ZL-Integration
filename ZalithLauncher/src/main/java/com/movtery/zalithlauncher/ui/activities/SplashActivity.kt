...
import com.movtery.zalithlauncher.components.UnpackGameResourcesTask
...
class SplashActivity : BaseAppCompatActivity() {
    ...
    private fun initUnpackItems() {
        ...
        // Keep the bundled game resources at the bottom of the list. It is also
        // executed after all launcher dependencies have finished installing.
        val gameResourcesTask = UnpackGameResourcesTask(this@SplashActivity)
        if (gameResourcesTask.assetExists()) {
            unpackItems.add(
                InstallableItem(
                    getString(R.string.unpack_screen_game_resources),
                    getString(R.string.unpack_screen_modpack_resources),
                    gameResourcesTask
                )
            )
        }
        ...
        lifecycleScope.launch {
            val jobs = unpackItems
                .filter {
                    ...
                    it.task !is UnpackGameResourcesTask &&
            ...
            // The modpack may depend on the launcher/runtime setup above, so it
            // deliberately runs as the final installation task.
            unpackItems.firstOrNull { it.task is UnpackGameResourcesTask }?.let { item ->
                val state = item.state.value
                if (state == InstallableItem.State.NOT_STARTED || state == InstallableItem.State.PENDING) {
                    item.updateState(InstallableItem.State.RUNNING)
                    runCatching {
